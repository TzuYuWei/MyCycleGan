...
class NonLocalBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        # 定義三個 1x1 卷積層
        self.theta = nn.Conv2d(in_channels, in_channels // 8, kernel_size=1, bias=False)
        self.phi = nn.Conv2d(in_channels, in_channels // 8, kernel_size=1, bias=False)
        self.g = nn.Conv2d(in_channels, in_channels // 2, kernel_size=1, bias=False)
    ...
    def forward(self, x): 
        ...
        assert h * w > 1, f"Feature map too small: got {h}x{w}, check network architecture!"

        # 轉換為二維特徵圖
        C_prime = c // 8  # `theta` 和 `phi` 的通道數
        theta = self.theta(x).view(b, C_prime, h * w)  # (b, C', N)
        phi = self.phi(x).view(b, C_prime, h * w)  # (b, C', N)

        # 計算 `attention_map`，確保 shape 是 `(b, N, N)`
        attention_map = self.softmax(torch.bmm(theta.permute(0, 2, 1), phi))  # (b, N, N)

        # 轉換 `g`，確保維度是 `(b, C'', N)`
        C_double_prime = c // 2  # `g` 的通道數
        g = self.g(x).view(b, C_double_prime, h * w)  # (b, C'', N)

        # `g` 和 `attention_map` 矩陣相乘，確保 shape 是 `(b, C'', N)`
        out = torch.bmm(g, attention_map)  # (b, C'', N)
        out = out.view(b, C_double_prime, h, w)  # 變回 (b, C'', H, W)

        out = self.conv(out)  # 經過 1x1 卷積回到原通道數
        return x + out  # 殘差連接

# === CSP 殘差塊 + NonLocal ===
class CSPResBlockWithNonLocal(nn.Module):
    def __init__(self, channels):
        super(CSPResBlockWithNonLocal, self).__init__()
        ...
        # 使用 Non-Local 模組
        self.nonlocal_block = NonLocalBlock(self.split_channels)

        self.final_conv = nn.Conv2d(channels, channels, kernel_size=1)

    def forward(self, x):
        x1, x2 = torch.chunk(x, 2, dim=1)  # CSP 分支
        out = self.main_path(x1)
        out = self.nonlocal_block(out)  # 非區域注意力
        out = x1 + 0.7 * out            # 殘差融合
        out = torch.cat([out, x2], dim=1)  # CSP 合併
        return self.final_conv(out)

# === Generator with CSP + NonLocal ===
class GeneratorWithCSPAttention(nn.Module):
    def __init__(self, input_channels=3, output_channels=3, num_res_blocks=6, width=512, height=256):
        ...
        self.width = width
        self.height = height
        self.res_blocks = nn.Sequential(*[CSPResBlockWithNonLocal(256) for _ in range(num_res_blocks)])
