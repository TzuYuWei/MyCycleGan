...
# === CSP 殘差塊 + CBAM ===
class CSPResBlockWithCBAM(nn.Module):
    def __init__(self, channels, reduction=16):
        super(CSPResBlockWithCBAM, self).__init__()
        ...
        self.cbam = CBAM(self.split_channels, reduction)
...
class GeneratorWithCSPAttention(nn.Module):
    def __init__(self, input_channels=3, output_channels=3, num_res_blocks=6, width=512, height=256):
        ...
        self.width = width
        self.height = height
        self.res_blocks = nn.Sequential(*[CSPResBlockWithCBAM(256) for _ in range(num_res_blocks)])
