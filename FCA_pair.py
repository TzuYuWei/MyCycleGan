...
# === CSP 殘差塊 + FCA ===
class CSPResBlockWithFCA(nn.Module):
    def __init__(self, channels, reduction=16, width=256, height=256):
        super(CSPResBlockWithFCA, self).__init__()
        ...
        self.fca = FcaLayer(self.split_channels, reduction, width, height)  # 使用 FCA 注意力
...
class GeneratorWithCSPAttention(nn.Module):
    def __init__(self, input_channels=3, output_channels=3, num_res_blocks=6, width=512, height=256):
        ...
        self.res_blocks = nn.Sequential(
            *[CSPResBlockWithFCA(256, reduction=16, width=self.width // 4, height=self.height // 4) for _ in range(num_res_blocks)]
        )
