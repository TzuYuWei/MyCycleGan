...
class GeneratorWithCSPAttention(nn.Module):
    def __init__(self, input_channels=3, output_channels=3, num_res_blocks=6, width=512, height=256):
        ...
        self.width = width
        self.height = height
        self.res_blocks = nn.Sequential(*[CSPResBlockWithSE(256) for _ in range(num_res_blocks)])
...
def train_rain_removal(generator_A2B, generator_B2A, discriminator_A, discriminator_B, paired_loader, device):
    ...
    for epoch in range(100):
        for i, (real_A, real_B, filename) in enumerate(paired_loader):
            ...
            if (epoch + 1):
                epoch_log_path = os.path.join(base_dir, "epoch_log.txt")
        ...
        if (epoch + 1) % 10 == 0:
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(loss_plot_dir, f"loss_curve_epoch{epoch+1}.png"))
            plt.close()
        ...
        print(f"Epoch [{epoch+1}/100], Iter [{i+1}] Loss G_A2B: {loss_G_A2B.item()} Loss G_B2A: {loss_G_B2A.item()} Loss D_A: {loss_D_A.item()} Loss D_B: {loss_D_B.item()}")
...
if __name__ == "__main__":
    ...
    output_dir = r'C:\Users\User\Desktop\小城市測試\R'
